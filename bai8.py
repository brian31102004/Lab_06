import pandas as pd

data_hd = {
    "MaHD": ["HD01","HD02","HD03","HD04","HD05","HD06","HD07","HD08","HD09","HD10"],
    "NgayBan": ["2026-04-01"]*2 + ["2026-04-02"]*2 + ["2026-04-03"]*2 + ["2026-04-04"]*2 + ["2026-04-05"]*2,
    "TenSP": ["Laptop","Chuot","Man hinh","USB","Loa","Tai nghe","Laptop","Webcam","Ban phim","Man hinh"],
    "SoLuong": [1, 5, 2, 10, 3, 4, 1, 2, 6, 1],
    "DonGia": [14500000,150000,2500000,180000,750000,450000,14500000,900000,300000,2500000],
    "NhanVien": ["An","Binh","An","Chi","Dung","Ha","An","Chi","Binh","Ha"]
}
df_hd = pd.DataFrame(data_hd)

# $ThanhTien = SoLuong \times DonGia$
df_hd["ThanhTien"] = df_hd["SoLuong"] * df_hd["DonGia"]

print("--- 5 hóa đơn cao nhất ---")
print(df_hd.nlargest(5, "ThanhTien"))

print("\n--- Hóa đơn trị giá >= 3,000,000 ---")
print(df_hd[df_hd["ThanhTien"] >= 3000000])

print("\nTổng doanh thu toàn bộ:", df_hd["ThanhTien"].sum(), "VND")